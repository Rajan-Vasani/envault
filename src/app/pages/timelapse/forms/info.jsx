...
export const TimelapseInfo = props => {
  ...
  const onFinish = values => {
    message.info(`Saving your timelapse`);
    saveNode(
      {
        type: 'timelapse',
        data: values,
      ...
      {
        onSuccess: () => {
          message.open({type: 'success', content: `Timelapse ${data?.id ? 'updated' : 'created'}`});
          queryClient.invalidateQueries({queryKey: [API_QUERY.NODE_DATA]});
        },
        onError: () => {
          message.open({type: 'error', content: `Could not ${data?.id ? 'update' : 'create'} timelapse`});
        },
      },
    );
  };
  ...
  return (
    <Form layout="vertical" name="form-timelapse" form={form} onFinish={onFinish} initialValues={{type: 'timelapse'}}>
