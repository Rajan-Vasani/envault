...
export const VariableInfo = props => {
  ...
  const onFinish = values => {
    message.info(`Saving your variable`);
    saveNode(
      {
        type: 'variable',
        data: values,
      ...
      {
        onSuccess: () => {
          message.open({type: 'success', content: `Variable ${data?.id ? 'updated' : 'created'}`});
          queryClient.invalidateQueries({queryKey: [API_QUERY.NODE_DATA]});
        },
        onError: () => {
          message.open({type: 'error', content: `Could not ${data?.id ? 'update' : 'create'} variable`});
        },
      },
    );
  };
  ...
  return (
    <Form layout="vertical" name="form-variable" form={form} onFinish={onFinish} initialValues={{type: 'variable'}}>
