...
export const TaskInfo = props => {
  ...
  const onFinish = values => {
    ...
    saveNode(
      ...
      {
        onSuccess: () => {
          message.open({type: 'success', content: `Task ${data?.id ? 'updated' : 'created'}`});
          queryClient.invalidateQueries({queryKey: [API_QUERY.NODE_DATA]});
        },
        onError: () => {
          message.open({type: 'error', content: `Could not ${data?.id ? 'update' : 'create'} task`});
        },
      },
    );
  };
  ...
  return (
    <Form
      ...
      form={form}
      onFinish={onFinish}
      initialValues={{type: 'task', actions: []}}
    >
